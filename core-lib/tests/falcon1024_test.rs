...
use core_lib::sig::falcon::falcon512::types::{Falcon512PublicKey, 
                                              Falcon512SecretKey, 
                                              Falcon512Signature};
...
fn falcon512_test_sign_verify_roundtrip() {
    ...
    let sign_result = Falcon1024::sign(TEST_MESSAGE_FALCON512, &sk);
    ...
    let is_valid = Falcon1024::verify(TEST_MESSAGE_FALCON512, &signature, &pk);
...
fn falcon512_test_verify_failure_wrong_public_key() {
    ...
    let sign_result = Falcon1024::sign(TEST_MESSAGE_FALCON512, &sk1);
    assert!(sign_result.is_ok(), "Falcon-512: Signing with sk1 failed: {:?}", sign_result.err());
    let signature = sign_result.unwrap();

    let is_valid = Falcon1024::verify(TEST_MESSAGE_FALCON512, &signature, &pk2);
...
fn falcon512_test_verify_failure_corrupted_signature() {
    ...
    let is_valid = Falcon1024::verify(TEST_MESSAGE_FALCON512, &corrupted_signature, &pk);
