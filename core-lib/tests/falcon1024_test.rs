...
use cypheron_core::sig::falcon::falcon1024::api::Falcon1024;
use cypheron_core::sig::falcon::falcon1024::constants::{
    FALCON_PUBLIC, FALCON_SECRET, FALCON_SIGNATURE,
};
use cypheron_core::sig::falcon::falcon1024::types::Signature;
...
fn falcon1024_test_keypair_generation_lengths() {
    ...
    assert!(
        result.is_ok(),
        "Falcon-1024: Keypair generation failed: {:?}",
        result.err()
    );
    let (pk, sk) = result.unwrap();

    println!(
        "Falcon-1024 Public Key generated successfully (len={})",
        pk.0.len()
    );
    println!(
        "Falcon-1024 Secret Key generated successfully (len={})",
    ...
    assert_eq!(
        pk.0.len(),
        FALCON_PUBLIC,
        "Falcon-1024: Public key length mismatch"
    );
    assert_eq!(
        sk.0.expose_secret().len(),
        FALCON_SECRET,
        "Falcon-1024: Secret key length mismatch"
    );
}
...
fn falcon1024_test_sign_verify_roundtrip() {
    ...
    assert!(
        ...
        "Falcon-1024: Keypair generation failed for roundtrip: {:?}",
        keypair_result.err()
    );
    ...
    println!(
        "Falcon-1024 Message: {:?}",
        std::str::from_utf8(TEST_MESSAGE_FALCON1024).unwrap_or("Invalid UTF-8")
    );
    println!(
        "Falcon-1024 Signature (len={}): {:02x?}",
        signature.0.len(),
        &signature.0[..32]
    );
    ...
    assert_eq!(
        signature.0.len(),
        FALCON_SIGNATURE,
        "Falcon-1024: Signature length mismatch"
    );

    let is_valid = Falcon1024::verify(TEST_MESSAGE_FALCON1024, &signature, &pk);
    assert!(
        is_valid,
        "Falcon-1024: Verification failed for a valid signature"
    );
}
...
fn falcon1024_test_verify_failure_wrong_message() {
    ...
    assert!(
        keypair_result.is_ok(),
        "Falcon-1024: Keypair generation failed for wrong message test: {:?}",
        keypair_result.err()
    );
    let (pk, sk) = keypair_result.unwrap();
    ...
    assert!(
        sign_result.is_ok(),
        "Falcon-1024: Signing failed for wrong message test: {:?}",
        sign_result.err()
    );
    let signature = sign_result.unwrap();
    ...
    assert!(
        !is_valid,
        "Falcon-1024: Verification should fail for a wrong message"
    );
}
...
fn falcon1024_test_verify_failure_wrong_public_key() {
    ...
    assert!(
        ...
        "Falcon-1024: Keypair 1 generation failed: {:?}",
        keypair1_result.err()
    );
    ...
    assert!(
        ...
        "Falcon-1024: Keypair 2 generation failed: {:?}",
        keypair2_result.err()
    );
    ...
    assert!(
        sign_result.is_ok(),
        "Falcon-1024: Signing with sk1 failed: {:?}",
        sign_result.err()
    );
    let signature = sign_result.unwrap();
    ...
    assert!(
        !is_valid,
        "Falcon-1024: Verification should fail for a wrong public key"
    );
}
...
fn falcon1024_test_verify_failure_corrupted_signature() {
    ...
    assert!(
        keypair_result.is_ok(),
        "Falcon-1024: Keypair generation failed for corrupted sig test: {:?}",
        keypair_result.err()
    );
    let (pk, sk) = keypair_result.unwrap();
    ...
    assert!(
        sign_result.is_ok(),
        "Falcon-1024: Signing failed for corrupted sig test: {:?}",
        sign_result.err()
    );
    ...
    assert!(
        !is_valid,
        "Falcon-1024: Verification should fail for a corrupted signature"
    );
}
