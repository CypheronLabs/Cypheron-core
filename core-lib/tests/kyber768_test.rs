...
fn test_variant_and_expose() -> TestResult<()> {
    test_assert_eq!(Kyber768::variant(), KemVariant::MlKem768, "KEM variant check");

    let (pk, sk) = crypto_operation!(Kyber768::keypair(), "keypair generation");
    let (ct, ss1) = crypto_operation!(Kyber768::encapsulate(&pk), "encapsulation");
    let ss2 = crypto_operation!(Kyber768::decapsulate(&ct, &sk), "decapsulation");
...
fn test_decapsulate_with_wrong_secret_key() -> TestResult<()> {
    let (pk1, _sk1) = crypto_operation!(Kyber768::keypair(), "first keypair generation");
    let (_pk2, sk2) = crypto_operation!(Kyber768::keypair(), "second keypair generation");
    let (ct, ss1) = crypto_operation!(Kyber768::encapsulate(&pk1), "encapsulation");
    let ss_wrong = crypto_operation!(Kyber768::decapsulate(&ct, &sk2), "decapsulation with wrong key");
...
fn test_decapsulate_with_corrupted_ciphertext() -> TestResult<()> {
    let (pk, sk) = crypto_operation!(Kyber768::keypair(), "keypair generation");
    let (mut ct, ss1) = crypto_operation!(Kyber768::encapsulate(&pk), "encapsulation");
    if !ct.is_empty() {
        ct[0] ^= 0xFF;
    }
    let ss_corrupt = crypto_operation!(Kyber768::decapsulate(&ct, &sk), "decapsulation with corrupted ciphertext");
