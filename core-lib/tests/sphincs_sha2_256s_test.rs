...
fn sha2256s_test_keypair_generation_lengths() {
    ...
    assert_eq!(pk.as_bytes().len(), types::PublicKey::length(), "SHA2-256s: Public key type length() mismatch");
    assert_eq!(sk.as_bytes().len(), types::SecretKey::length(), "SHA2-256s: Secret key type length() mismatch");
}
...
fn sha2256s_test_keypair_from_seed_lengths_and_determinism() {
    ...
    let (pk1_s1, sk1_s1) = keypair_from_seed(&seed_data1).expect("SHA2-256s: Keypair from seed 1 failed");
    assert_eq!(pk1_s1.as_bytes().len(), public_key_bytes(), "SHA2-256s: PK length from seed mismatch");
    assert_eq!(sk1_s1.as_bytes().len(), secret_key_bytes(), "SHA2-256s: SK length from seed mismatch");

    let (pk2_s1, sk2_s1) = keypair_from_seed(&seed_data1).expect("SHA2-256s: Keypair from seed 1 (deterministic) failed");
    assert_eq!(pk1_s1.as_bytes(), pk2_s1.as_bytes(), "SHA2-256s: Public keys from same seed should be identical");
    assert_eq!(sk1_s1.as_bytes(), sk2_s1.as_bytes(), "SHA2-256s: Secret keys from same seed should be identical");

    let (pk_s2, _sk_s2) = keypair_from_seed(&seed_data2).expect("SHA2-256s: Keypair from seed 2 failed");
    assert_ne!(pk1_s1.as_bytes(), pk_s2.as_bytes(), "SHA2-256s: Public keys from different seeds should be different");
}
...
fn sha2256s_test_sign_verify_detached_roundtrip() {
    ...
    assert_eq!(signature.as_bytes().len(), signature_bytes(), "SHA2-256s: Signature length mismatch");
    assert_eq!(signature.as_bytes().len(), types::Signature::length(), "SHA2-256s: Signature type length() mismatch");

    let verification_result = verify_detached(&signature, TEST_MESSAGE_SHA2_256S, &pk);
    assert!(verification_result.is_ok(), "SHA2-256s: Verification failed for a valid signature: {:?}", verification_result.err());
}
...
fn sha2256s_test_verify_detached_failure_wrong_message() {
    ...
    assert!(verification_result.is_err(), "SHA2-256s: Verification should fail for a wrong message");
...
fn sha2256s_test_verify_detached_failure_wrong_public_key() {
    ...
    let signature = sign_detached(TEST_MESSAGE_SHA2_256S, &sk1).expect("SHA2-256s: Signing with sk1 failed");

    let verification_result = verify_detached(&signature, TEST_MESSAGE_SHA2_256S, &pk2);
    assert!(verification_result.is_err(), "SHA2-256s: Verification should fail for a wrong public key");
    match verification_result.err().unwrap() {
        SphincsError::VerificationFailed => { /* Expected error */ },
...
fn sha2256s_test_verify_detached_failure_corrupted_signature() {
    let (pk, sk) = keypair().expect("SHA2-256s: Keypair generation failed");
    let mut signature_bytes_vec = sign_detached(TEST_MESSAGE_SHA2_256S, &sk).expect("SHA2-256s: Signing failed").as_bytes().to_vec();
    ...
    assert!(verification_result.is_err(), "SHA2-256s: Verification should fail for a corrupted signature");
    match verification_result.err().unwrap() {
        SphincsError::VerificationFailed => { /* Expected error */ },
...
fn sha2256s_test_sign_open_combined_roundtrip() {
    ...
    let signed_message = sign_combined(TEST_MESSAGE_SHA2_256S, &sk).expect("SHA2-256s: Combined signing failed");

    assert!(signed_message.len() > TEST_MESSAGE_SHA2_256S.len(), "SHA2-256s: Combined message should be longer");
    assert_eq!(signed_message.len(), TEST_MESSAGE_SHA2_256S.len() + signature_bytes(), "SHA2-256s: Combined message length incorrect");

    let opened_message_result = open_combined(&signed_message, &pk);
    assert!(opened_message_result.is_ok(), "SHA2-256s: Opening combined message failed: {:?}", opened_message_result.err());

    let opened_message = opened_message_result.unwrap();
    assert_eq!(opened_message, TEST_MESSAGE_SHA2_256S, "SHA2-256s: Opened message does not match original message");
}
...
fn sha2256s_test_open_combined_failure_wrong_pk() {
    ...
    let signed_message = sign_combined(TEST_MESSAGE_SHA2_256S, &sk1).expect("SHA2-256s: Combined signing failed");

    let opened_message_result = open_combined(&signed_message, &pk2);
    assert!(opened_message_result.is_err(), "SHA2-256s: Opening combined message should fail with wrong PK");
    ...
    if seed_len > 0 { 
        ...
        match result.err().unwrap() {
            SphincsError::InvalidSeedLength { expected, actual } => {
                ...
                assert_eq!(actual, invalid_seed_data.len(), "SHA2-256s: Actual seed length mismatch in error");
            },
