use core_lib::sig::sphincs::haraka_192f::api::*;
use core_lib::sig::sphincs::haraka_192f::types;
use core_lib::sig::sphincs::errors::SphincsError;

const TEST_MESSAGE: &[u8] = b"Test message for Haraka-192f.";

#[test]
fn haraka192f_test_keypair_generation_lengths() {
    let (pk, sk) = keypair().expect("Haraka-192f keypair generation failed");

    assert_eq!(pk.as_bytes().len(), public_key_bytes());
    assert_eq!(sk.as_bytes().len(), secret_key_bytes());
    
    assert_eq!(pk.as_bytes().len(), types::PublicKey::length());
    assert_eq!(sk.as_bytes().len(), types::SecretKey::length());
}
#[test]
fn haraka192f_test_keypair_from_seed_lengths_and_determinism() {
    ...
    let (pk2_s1, sk2_s1) = keypair_from_seed(&seed_data1).expect("Keypair from seed 1 (deterministic) failed");
    assert_eq!(pk1_s1.as_bytes(), pk2_s1.as_bytes(), "Public keys from same seed should be identical");
    assert_eq!(sk1_s1.as_bytes(), sk2_s1.as_bytes(), "Secret keys from same seed should be identical");

    let (pk_s2, _sk_s2) = keypair_from_seed(&seed_data2).expect("Keypair from seed 2 failed");
    assert_ne!(pk1_s1.as_bytes(), pk_s2.as_bytes(), "Public keys from different seeds should be different");
}

#[test]
fn haraka192f_test_sign_verify_detached_roundtrip() {
    ...
    assert!(verification_result.is_ok(), "Verification failed for a valid signature: {:?}", verification_result.err());
}

#[test]
fn haraka192f_test_verify_detached_failure_wrong_message() {
    let (pk, sk) = keypair().expect("Keypair generation failed");
    let signature = sign_detached(TEST_MESSAGE, &sk).expect("Signing failed");
...
#[test]
fn haraka192f_test_verify_detached_failure_wrong_public_key() {
...
fn haraka192f_test_verify_detached_failure_corrupted_signature() {
    let (pk, sk) = keypair().expect("Keypair generation failed");
    let mut signature_bytes_vec = sign_detached(TEST_MESSAGE, &sk).expect("Signing failed").as_bytes().to_vec();
...
#[test]
fn haraka192f_test_sign_open_combined_roundtrip() {
    let (pk, sk) = keypair().expect("Keypair generation failed");
    ...
    assert!(opened_message_result.is_ok(), "Opening combined message failed: {:?}", opened_message_result.err());
...
#[test]
fn haraka192f_test_open_combined_failure_wrong_pk() {
    let (_pk1, sk1) = keypair().expect("Keypair 1 generation failed");
    let (pk2, _sk2) = keypair().expect("Keypair 2 generation failed"); 
...
#[test]
fn haraka192f_test_invalid_seed_length() {
    let seed_len = seed_bytes();
