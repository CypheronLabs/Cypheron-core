...
fn shake192f_test_keypair_generation_lengths() {
    ...
    assert_eq!(pk.as_bytes().len(), types::PublicKey::length(), "Public key type length() mismatch");
    assert_eq!(sk.as_bytes().len(), types::SecretKey::length(), "Secret key type length() mismatch");
}
