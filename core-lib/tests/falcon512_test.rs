...
use cypheron_core::sig::falcon::falcon512::constants::{
...
fn falcon512_test_keypair_generation_lengths() {
    ...
    println!(
        "Falcon-512 Public Key generated successfully (len={})",
        pk.0.len()
    );
    println!(
        "Falcon-512 Secret Key generated successfully (len={})",
    ...
    assert_eq!(
        pk.0.len(),
        FALCON_PUBLIC,
        "Falcon-512: Public key length mismatch"
    );
    assert_eq!(
        sk.0.expose_secret().len(),
        FALCON_SECRET,
        "Falcon-512: Secret key length mismatch"
    );
}
...
fn falcon512_test_sign_verify_roundtrip() {
    ...
    assert_eq!(
        signature.0.len(),
        FALCON_SIGNATURE,
        "Falcon-512: Signature length mismatch"
    );

    let is_valid = Falcon512::verify(TEST_MESSAGE_FALCON512, &signature, &pk);
    assert!(
        is_valid,
        "Falcon-512: Verification failed for a valid signature"
    );
}
...
fn falcon512_test_verify_failure_wrong_message() {
    ...
    assert!(
        !is_valid,
        "Falcon-512: Verification should fail for a wrong message"
    );
}
...
fn falcon512_test_verify_failure_wrong_public_key() {
    ...
    assert!(
        sign_result.is_ok(),
        "Falcon-512: Signing with sk1 failed: {:?}",
        sign_result.err()
    );
    let signature = sign_result.unwrap();
    ...
    assert!(
        !is_valid,
        "Falcon-512: Verification should fail for a wrong public key"
    );
}
...
fn falcon512_test_verify_failure_corrupted_signature() {
    ...
    assert!(
        !is_valid,
        "Falcon-512: Verification should fail for a corrupted signature"
    );
}
