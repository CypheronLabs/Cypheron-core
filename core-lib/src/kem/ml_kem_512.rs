...
impl Kem for MlKem512 {
    ...
    fn keypair() -> Result<(Self::PublicKey, Self::SecretKey), Self::Error> {
        ...
        validate_ffi_kem_buffer!(&pk, sizes::ML_KEM_512_PUBLIC);
        validate_ffi_kem_buffer!(&sk, sizes::ML_KEM_512_SECRET);
    ...
    fn encapsulate(
        ...
        validate_ffi_kem_buffer!(&pk.0, sizes::ML_KEM_512_PUBLIC);

        let mut ct = vec![0u8; sizes::ML_KEM_512_CIPHERTEXT];
        let mut ss = [0u8; sizes::ML_KEM_512_SHARED];

        validate_ffi_kem_buffer!(&ct, sizes::ML_KEM_512_CIPHERTEXT);
        validate_ffi_kem_buffer!(&ss, sizes::ML_KEM_512_SHARED);
    ...
    fn decapsulate(
        ...
        validate_ffi_kem_buffer!(sk_bytes, sizes::ML_KEM_512_SECRET);
        ...
        let result = unsafe {
            pqcrystals_kyber512_ref_dec(ss.as_mut_ptr(), ct.as_ptr(), sk_bytes.as_ptr())
        };
