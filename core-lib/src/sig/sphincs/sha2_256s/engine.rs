...
static ENG_PUBLIC_KEY_BYTES: Lazy<usize> =
    Lazy::new(|| unsafe { ffi::crypto_sign_publickeybytes() as usize });
static ENG_SECRET_KEY_BYTES: Lazy<usize> =
    Lazy::new(|| unsafe { ffi::crypto_sign_secretkeybytes() as usize });
static ENG_SIGNATURE_BYTES: Lazy<usize> =
    Lazy::new(|| unsafe { ffi::crypto_sign_bytes() as usize });
static ENG_SEED_BYTES: Lazy<usize> =
    Lazy::new(|| unsafe { ffi::crypto_sign_seedbytes() as usize });

pub fn public_key_bytes() -> usize { *ENG_PUBLIC_KEY_BYTES }
pub fn secret_key_bytes() -> usize { *ENG_SECRET_KEY_BYTES }
pub fn signature_bytes() -> usize { *ENG_SIGNATURE_BYTES }
pub fn seed_bytes() -> usize { *ENG_SEED_BYTES }
...
pub fn keypair_from_seed_generate(seed: &Seed) -> Result<(PublicKey, SecretKey), SphincsError> {
    ...
    if ret_code == 0 { Ok((pk, sk)) } else { Err(SphincsError::KeyPairGenerationFailed(ret_code)) }
}
