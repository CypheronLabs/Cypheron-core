...
static PUBLIC_KEY_BYTES_REF: Lazy<usize> = Lazy::new(|| 
    unsafe {
        ffi::crypto_sign_publickeybytes() as usize
    });
static SECRET_KEY_BYTES_REF: Lazy<usize> = Lazy::new(||
    unsafe {
        ffi::crypto_sign_secretkeybytes() as usize
    });
static SIGNATURE_BYTES_REF: Lazy<usize> = Lazy::new(||
    unsafe {
        ffi::crypto_sign_bytes() as usize
    });

static SEED_BYTES_REF: Lazy<usize> = Lazy::new(||
    unsafe {
        ffi::crypto_sign_seedbytes() as usize
    });
...
impl PublicKey {
    ...
    pub fn from_bytes(bytes: &[u8]) -> Result<Self, SphincsError> {
        ...
        if bytes.len() != expected_len {
            Err(SphincsError::InvalidPublicKeyLength { expected: expected_len, actual: bytes.len() })
        } else {
    ...
    pub fn length() -> usize { *PUBLIC_KEY_BYTES_REF }
}
...
impl fmt::Debug for PublicKey {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("PublicKeySha2_256fRobust") // <--- Optional: Specific debug name
            ...
            .field("bytes_prefix", &self.0.get(..std::cmp::min(self.0.len(), 8)).unwrap_or_default())
