...
pub struct Dilithium3Engine;

impl SignatureEngine for Dilithium3Engine {
    ...
    fn keypair() -> Result<(Self::PublicKey, Self::SecretKey), Self::Error> {
        ...
        validate_ffi_dilithium_buffer!(&pk, ML_DSA_65_PUBLIC);
        validate_ffi_dilithium_buffer!(&sk, ML_DSA_65_SECRET);
    ...
    fn sign(msg: &[u8], sk: &Self::SecretKey) -> Result<Self::Signature, Self::Error> {
        ...
        let mut sig_buffer = [0u8; ML_DSA_65_SIGNATURE];
        let mut siglen = 0usize;
        let sk_bytes = sk.0.expose_secret();

        validate_ffi_dilithium_buffer!(sk_bytes, ML_DSA_65_SECRET);
        validate_ffi_dilithium_buffer!(&sig_buffer, ML_DSA_65_SIGNATURE);
        ...
        match result {
            0 => {
                validate_dilithium_signature_output!(sig_buffer, siglen, ML_DSA_65_SIGNATURE);
    ...
    fn verify(msg: &[u8], sig: &Self::Signature, pk: &Self::PublicKey) -> bool {
        ...
        if sig.0.len() != ML_DSA_65_SIGNATURE {
            return false;
        }
        if pk.0.len() != ML_DSA_65_PUBLIC {
            return false;
        }
