...
use crate::utils::encoding::{encode_base64_url, decode_base64_url};
...
pub async fn sign_hybrid_jwt(Json(payload): Json<HybridSignRequest>) -> Result<impl IntoResponse, AppError> {
    let header = json!({
    ...
    let header_base64 = encode_base64_url(&serde_json::to_vec(&header).map_err(|_| AppError::InvalidBase64)?);
    let payload_base64 = encode_base64_url(&serde_json::to_vec(&payload_json).map_err(|_| AppError::InvalidBase64)?);
    let signing_input = format!("{}.{}", header_base64, payload_base64);

    let es256_sk_bytes = decode_base64_url(&payload.es256)?;
    let es256_sk_array: [u8; 32] = es256_sk_bytes
        .try_into()
        .map_err(|_| AppError::InvalidSecretKey)?;

    let es256_sk = SigningKey::from_bytes((&es256_sk_array).into())
        .map_err(|_| AppError::InvalidSecretKey)?;
    ...
    let sigs_b64 = encode_base64_url(&serde_json::to_vec(&sigs_json).map_err(|_| AppError::InvalidBase64)?);
