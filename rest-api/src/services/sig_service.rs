...
use core_lib::sig::sphincs::haraka_192f::types::{
    PublicKey as Haraka192fPublicKey, SecretKey as Haraka192fSecretKey,
    Signature as Haraka192fSignature,
};
use core_lib::sig::sphincs::sha2_256s::types::{
    PublicKey as Sha2_256sPublicKey, SecretKey as Sha2_256sSecretKey,
    Signature as Sha2_256sSignature,
};
use core_lib::sig::sphincs::shake_128f::types::{
    PublicKey as Shake128fPublicKey, SecretKey as Shake128fSecretKey,
    Signature as Shake128fSignature,
};
...
impl SigService {
    ...
    pub fn sign(
        ...
        message: &str,
        sk_b64: &str,
    ) -> Result<AnySignature, AppError> {
        validation::validate_message(message)?;
        validation::validate_base64_key(sk_b64)?;

        let sk_bytes =
            general_purpose::STANDARD.decode(sk_b64).map_err(|_| AppError::InvalidSecretKey)?;
        ...
        match variant {
            SigVariant::MlDsa44 => {
                ...
                let arr: [u8; ML_DSA_44_SECRET] =
                    sk_bytes.try_into().map_err(|_| AppError::InvalidSecretKey)?;
                let sk = SecretKey(SecretBox::new(Box::new(arr)));
                let sig =
                    Dilithium2::sign(message_bytes, &sk).map_err(|_| AppError::SigningFailed)?;
                Ok(AnySignature::Dilithium2(sig))
            }
            SigVariant::MlDsa65 => {
                use core_lib::sig::dilithium::dilithium3::types::SecretKey;
                use secrecy::SecretBox;
                let arr: [u8; ML_DSA_65_SECRET] =
                    sk_bytes.try_into().map_err(|_| AppError::InvalidSecretKey)?;
                let sk = SecretKey(SecretBox::new(Box::new(arr)));
                let sig =
                    Dilithium3::sign(message_bytes, &sk).map_err(|_| AppError::SigningFailed)?;
                Ok(AnySignature::Dilithium3(sig))
            }
            SigVariant::MlDsa87 => {
                use core_lib::sig::dilithium::dilithium5::types::SecretKey;
                use secrecy::SecretBox;
                let arr: [u8; ML_DSA_87_SECRET] =
                    sk_bytes.try_into().map_err(|_| AppError::InvalidSecretKey)?;
                let sk = SecretKey(SecretBox::new(Box::new(arr)));
                let sig =
                    Dilithium5::sign(message_bytes, &sk).map_err(|_| AppError::SigningFailed)?;
                Ok(AnySignature::Dilithium5(sig))
            }
            SigVariant::Falcon512 => {
                use core_lib::sig::falcon::falcon512::types::SecretKey as Falcon512SecretKey;
                use secrecy::SecretBox;
                let arr: [u8; core_lib::sig::falcon::falcon512::constants::FALCON_SECRET] =
                    sk_bytes.try_into().map_err(|_| AppError::InvalidSecretKey)?;
                let sk = Falcon512SecretKey(SecretBox::new(Box::new(arr)));
                let sig =
                    Falcon512::sign(message_bytes, &sk).map_err(|_| AppError::SigningFailed)?;
                Ok(AnySignature::Falcon512(sig))
            }
            SigVariant::Falcon1024 => {
                use core_lib::sig::falcon::falcon1024::types::SecretKey as Falcon1024SecretKey;
                use secrecy::SecretBox;
                let arr: [u8; FALCON_SECRET] =
                    sk_bytes.try_into().map_err(|_| AppError::InvalidSecretKey)?;
                let sk = Falcon1024SecretKey(SecretBox::new(Box::new(arr)));
                let sig =
                    Falcon1024::sign(message_bytes, &sk).map_err(|_| AppError::SigningFailed)?;
                Ok(AnySignature::Falcon1024(sig))
            }
            SigVariant::SlhDsaHaraka192f => {
                let sk = Haraka192fSecretKey::from_bytes(&sk_bytes)
                    .map_err(|_| AppError::InvalidSecretKey)?;
                let sig = haraka_192f::sign_detached(message_bytes, &sk)
                    .map_err(|_| AppError::SigningFailed)?;
                Ok(AnySignature::Haraka192f(sig))
            }
            SigVariant::SlhDsaSha2256s => {
                let sk = Sha2_256sSecretKey::from_bytes(&sk_bytes)
                    .map_err(|_| AppError::InvalidSecretKey)?;
                let sig = sha2_256s::sign_detached(message_bytes, &sk)
                    .map_err(|_| AppError::SigningFailed)?;
                Ok(AnySignature::Sha2_256s(sig))
            }
            SigVariant::SlhDsaShake128f => {
                let sk = Shake128fSecretKey::from_bytes(&sk_bytes)
                    .map_err(|_| AppError::InvalidSecretKey)?;
                let sig = shake_128f::sign_detached(message_bytes, &sk)
                    .map_err(|_| AppError::SigningFailed)?;
    ...
    pub fn verify(
        ...
        let pk_bytes =
            general_purpose::STANDARD.decode(pk_b64).map_err(|_| AppError::InvalidBase64)?;
        let sig_bytes =
            general_purpose::STANDARD.decode(sig_b64).map_err(|_| AppError::InvalidBase64)?;
        ...
        match variant {
            SigVariant::MlDsa44 => {
                let pk = core_lib::sig::dilithium::dilithium2::types::PublicKey(
                    pk_bytes.try_into().map_err(|_| AppError::InvalidPublicKey)?,
                );
                let sig = core_lib::sig::dilithium::dilithium2::types::Signature(
                    sig_bytes.try_into().map_err(|_| AppError::InvalidSignature)?,
                );
                Ok(Dilithium2::verify(msg_bytes, &sig, &pk))
            }
            SigVariant::MlDsa65 => {
                let pk = core_lib::sig::dilithium::dilithium3::types::PublicKey(
                    pk_bytes.try_into().map_err(|_| AppError::InvalidPublicKey)?,
                );
                let sig = core_lib::sig::dilithium::dilithium3::types::Signature(
                    sig_bytes.try_into().map_err(|_| AppError::InvalidSignature)?,
                );
                Ok(Dilithium3::verify(msg_bytes, &sig, &pk))
            }
            SigVariant::MlDsa87 => {
                let pk = core_lib::sig::dilithium::dilithium5::types::PublicKey(
                    pk_bytes.try_into().map_err(|_| AppError::InvalidPublicKey)?,
                );
                let sig = core_lib::sig::dilithium::dilithium5::types::Signature(
                    sig_bytes.try_into().map_err(|_| AppError::InvalidSignature)?,
                );
            ...
            SigVariant::Falcon512 => {
                ...
                let pk = Falcon512PublicKey(
                    pk_bytes.try_into().map_err(|_| AppError::InvalidPublicKey)?,
                );
                let sig = Falcon512Signature(
                    sig_bytes.try_into().map_err(|_| AppError::InvalidSignature)?,
                );
                Ok(Falcon512::verify(msg_bytes, &sig, &pk))
            }
            SigVariant::Falcon1024 => {
                use core_lib::sig::falcon::falcon1024::types::{
                    PublicKey as Falcon1024PublicKey, Signature as Falcon1024Signature,
                };
                let pk = Falcon1024PublicKey(
                    pk_bytes.try_into().map_err(|_| AppError::InvalidPublicKey)?,
                );
                let sig = Falcon1024Signature(
                    sig_bytes.try_into().map_err(|_| AppError::InvalidSignature)?,
                );
                Ok(Falcon1024::verify(msg_bytes, &sig, &pk))
            }
            SigVariant::SlhDsaHaraka192f => {
                let pk = Haraka192fPublicKey::from_bytes(&pk_bytes)
                    .map_err(|_| AppError::InvalidPublicKey)?;
                let sig = Haraka192fSignature::from_bytes(&sig_bytes)
                    .map_err(|_| AppError::InvalidSignature)?;
                Ok(haraka_192f::verify_detached(&sig, msg_bytes, &pk).is_ok())
            }
            SigVariant::SlhDsaSha2256s => {
                let pk = Sha2_256sPublicKey::from_bytes(&pk_bytes)
                    .map_err(|_| AppError::InvalidPublicKey)?;
                let sig = Sha2_256sSignature::from_bytes(&sig_bytes)
                    .map_err(|_| AppError::InvalidSignature)?;
                Ok(sha2_256s::verify_detached(&sig, msg_bytes, &pk).is_ok())
            }
            SigVariant::SlhDsaShake128f => {
                let pk = Shake128fPublicKey::from_bytes(&pk_bytes)
                    .map_err(|_| AppError::InvalidPublicKey)?;
                let sig = Shake128fSignature::from_bytes(&sig_bytes)
                    .map_err(|_| AppError::InvalidSignature)?;
...
pub enum AnySignature {
    ...
    Falcon512(
        core_lib::sig::falcon::falcon512::types::Signature<
            { core_lib::sig::falcon::falcon512::constants::FALCON_SIGNATURE },
        >,
    ),
    Falcon1024(
        core_lib::sig::falcon::falcon1024::types::Signature<
            { core_lib::sig::falcon::falcon1024::constants::FALCON_SIGNATURE },
        >,
    ),
