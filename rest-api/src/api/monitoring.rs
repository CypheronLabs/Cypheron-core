...
pub fn routes() -> Router<MonitoringState> {
    Router::new()
        ...
        .route(
            ...
            post(monitoring_handler::acknowledge_alert),
        )
        ...
        .route(
            "/monitoring/security/anomalies",
            get(monitoring_handler::get_anomaly_detection_results),
        )
        // Compliance monitoring
        .route("/monitoring/compliance/report", get(monitoring_handler::get_compliance_report))
        .route(
            "/monitoring/compliance/dashboard",
            get(monitoring_handler::get_compliance_dashboard),
        )
        .route(
            "/monitoring/compliance/assess",
            post(monitoring_handler::assess_compliance_framework),
        )
}
