...
fn benchmark_complete_signature_workflows(c: &mut Criterion) {
    ...
    group.bench_function("SPHINCS+-SHAKE-128f Complete Workflow", |b| {
        b.iter(|| {
            ...
            let signature =
                core_lib::sig::sphincs::shake_128f::sign_detached(black_box(test_message), &sk)
...
fn benchmark_hybrid_complete_workflows(c: &mut Criterion) {
    ...
    group.bench_function("ECC+Dilithium Complete Workflow", |b| {
        b.iter(|| {
            ...
            let signature =
                EccDilithium::sign(black_box(test_message), &sk).expect("Hybrid signing failed");
    ...
    group.bench_function("ECC+Falcon Complete Workflow", |b| {
        b.iter(|| {
            ...
            let signature =
                EccFalcon::sign(black_box(test_message), &sk).expect("Hybrid signing failed");
    ...
    group.bench_function("ECC+SPHINCS+ Complete Workflow", |b| {
        b.iter(|| {
            ...
            let signature =
                EccSphincs::sign(black_box(test_message), &sk).expect("Hybrid signing failed");
...
fn benchmark_real_world_scenarios(c: &mut Criterion) {
    ...
    group.bench_function("Secure Communication Setup (KEM + Signature)", |b| {
        ...
        b.iter(|| {
            let (alice_kem_pk, alice_kem_sk) =
                MlKem768::keypair().expect("Failed to generate KEM keys");
            let (alice_sig_pk, alice_sig_sk) =
                MlDsa44::keypair().expect("Failed to generate signature keys");

            let (bob_kem_pk, bob_kem_sk) =
                MlKem768::keypair().expect("Failed to generate KEM keys");
            let (bob_sig_pk, _bob_sig_sk) =
                MlDsa44::keypair().expect("Failed to generate signature keys");

            let (ciphertext, shared_secret) =
                MlKem768::encapsulate(&bob_kem_pk).expect("Failed to encapsulate");
            let signature = MlDsa44::sign(document, &alice_sig_sk).expect("Failed to sign");

            let decrypted_secret =
                MlKem768::decapsulate(&ciphertext, &bob_kem_sk).expect("Failed to decapsulate");
            let is_valid = MlDsa44::verify(document, &signature, &alice_sig_pk);

            assert_eq!(
                MlKem768::expose_shared(&shared_secret),
                MlKem768::expose_shared(&decrypted_secret)
            );
            assert!(is_valid);
            black_box(())
        })
        ...
        b.iter(|| {
            ...
            for i in 0..parties.len() {
                for j in (i + 1)..parties.len() {
                    let (ct, ss) =
                        MlKem768::encapsulate(&parties[j].0).expect("Failed to encapsulate");
                    let ss_decrypted =
                        MlKem768::decapsulate(&ct, &parties[j].1).expect("Failed to decapsulate");

                    assert_eq!(
                        MlKem768::expose_shared(&ss),
                        MlKem768::expose_shared(&ss_decrypted)
                    );
...
fn benchmark_mixed_algorithm_scenarios(c: &mut Criterion) {
    ...
    group.bench_function("Mixed Security Levels Workflow", |b| {
        ...
        b.iter(|| {
            ...
            let (main_ct, main_ss) =
                MlKem1024::encapsulate(&kem_pk).expect("Failed to encapsulate");
            let signature = MlDsa44::sign(document, &sig_sk).expect("Signing failed");
            let (temp_ct, temp_ss) =
                MlKem512::encapsulate(&temp_pk).expect("Failed to encapsulate");

            let main_ss_dec =
                MlKem1024::decapsulate(&main_ct, &kem_sk).expect("Failed to decapsulate");
            let sig_valid = MlDsa44::verify(document, &signature, &sig_pk);
            let temp_ss_dec =
                MlKem512::decapsulate(&temp_ct, &temp_sk).expect("Failed to decapsulate");

            assert_eq!(
                MlKem1024::expose_shared(&main_ss),
                MlKem1024::expose_shared(&main_ss_dec)
            );
            assert!(sig_valid);
            assert_eq!(
                MlKem512::expose_shared(&temp_ss),
                MlKem512::expose_shared(&temp_ss_dec)
            );
    ...
    group.bench_function("Hybrid + Pure PQC Combination", |b| {
        ...
        b.iter(|| {
            ...
            let (pqc_kem_pk, pqc_kem_sk) =
                MlKem1024::keypair().expect("Failed to generate keypair");

            let (pqc_sig_pk, pqc_sig_sk) = MlDsa87::keypair().expect("Key generation failed");

            let hybrid_sig =
                EccDilithium::sign(message, &hybrid_sk).expect("Hybrid signing failed");
            let (kem_ct, kem_ss) =
                MlKem1024::encapsulate(&pqc_kem_pk).expect("Failed to encapsulate");
            let pqc_sig = MlDsa87::sign(message, &pqc_sig_sk).expect("Signing failed");

            let hybrid_valid = EccDilithium::verify(message, &hybrid_sig, &hybrid_pk);
            let kem_ss_dec =
                MlKem1024::decapsulate(&kem_ct, &pqc_kem_sk).expect("Failed to decapsulate");
            let pqc_sig_valid = MlDsa87::verify(message, &pqc_sig, &pqc_sig_pk);

            assert!(hybrid_valid);
            assert_eq!(
                MlKem1024::expose_shared(&kem_ss),
                MlKem1024::expose_shared(&kem_ss_dec)
            );
...
fn benchmark_resource_constrained_scenarios(c: &mut Criterion) {
    ...
    group.bench_function("Time-Critical Operations", |b| {
        ...
        b.iter(|| {
            ...
            let signature =
                MlDsa44::sign(black_box(message), black_box(&sig_sk)).expect("Signing failed");

            let ss2 = MlKem512::decapsulate(black_box(&ct), black_box(&sk))
                .expect("Failed to decapsulate");
            let valid = MlDsa44::verify(
                black_box(message),
                black_box(&signature),
                black_box(&sig_pk),
            );
