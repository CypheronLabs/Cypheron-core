...
fn benchmark_baseline_signature_operations(c: &mut Criterion) {
    ...
    group.bench_function("Baseline SPHINCS+-SHAKE-128f Full Operation", |b| {
        b.iter(|| {
            ...
            let result =
                core_lib::sig::sphincs::shake_128f::verify_detached(&signature, message, &pk);
            assert!(result.is_ok());
            black_box(())
        })
    });
    ...
    group.bench_function("Cross-Platform ML-KEM-768 Consistency", |b| {
        b.iter(|| {
            ...
            for _ in 0..10 {
                ...
                if MlKem768::expose_shared(&ss1_1) == MlKem768::expose_shared(&ss1_2) || ct1 == ct2
                {
                    all_consistent = false;
                }
            }
            ...
            assert!(
                all_consistent,
                "Cross-platform consistency regression detected"
            );
        ...
        b.iter(|| {
            ...
            for i in 0..public_keys.len() {
                for j in (i + 1)..public_keys.len() {
                    assert_ne!(
                        public_keys[i].0, public_keys[j].0,
                        "Public key uniqueness regression detected"
                    );
                    assert_ne!(
                        MlKem768::expose_shared(&shared_secrets[i]),
...
fn benchmark_backwards_compatibility(c: &mut Criterion) {
    ...
    group.bench_function("Key Format Compatibility", |b| {
        b.iter(|| {
            ...
            assert_eq!(
                ...
                2400,
                "ML-KEM-768 secret key size changed"
            );
            ...
            assert_eq!(
                ...
                32,
                "ML-KEM-768 shared secret size changed"
            );
    ...
    group.bench_function("Hybrid Format Compatibility", |b| {
        ...
        b.iter(|| {
            ...
            let (kem_pk, kem_sk) =
                P256MlKem768::keypair().expect("Hybrid KEM key generation failed");
            let (ct, ss1) =
                P256MlKem768::encapsulate(&kem_pk).expect("Hybrid encapsulation failed");
            let ss2 = P256MlKem768::decapsulate(&ct, &kem_sk).expect("Hybrid decapsulation failed");

            assert_eq!(ss1.as_bytes(), ss2.as_bytes());

            assert_eq!(
                kem_pk.classical.0.len(),
                65,
                "P-256 public key size changed"
            );
            assert_eq!(
                kem_pk.post_quantum.0.len(),
                1184,
                "ML-KEM-768 in hybrid public key size changed"
            );
            assert_eq!(
                ct.classical_ephemeral.len(),
                65,
                "P-256 ephemeral key size changed"
            );
            assert_eq!(
                ct.post_quantum_ciphertext.len(),
                1088,
                "ML-KEM-768 in hybrid ciphertext size changed"
            );
            assert_eq!(
                ss1.as_bytes().len(),
                32,
                "Hybrid shared secret size changed"
            );
